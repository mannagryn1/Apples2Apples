...
class Player {
	...
	public ArrayListString hand;
	public ArrayListString greenApples = new ArrayListString();
	public Player(int playerID, ArrayListString hand, boolean isBot) {
	...
	public void play() {
		if(isBot) {
			...
			hand.remove(choice);
			System.out.println(Waiting for other playersn);	
		}
	}
	...
	public PlayedApple judge() {
		if(isBot){
			...
			System.out.println(Choose which red apple winsn);
			int choice = 0;
			try {
				BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
				String input=br.readLine();
	...
	public void addCard(String redApple) {
		if(isBot  !online) {
			...
			try {
				outToClient.writeBytes(redApple + n);
			} catch (Exception e){}
		}
	}
}
...
public class Apples2Apples {
	public ArrayListString redApples;
	public ArrayListString greenApples;
	public static ArrayListPlayedApple playedApple = new ArrayListPlayedApple();
	public ArrayListPlayer players = new ArrayListPlayer();
	public Random rnd;
	...
	public Apples2Apples(String ipAddress) throws Exception {
		...
		ArrayListString hand = new ArrayListString(Arrays.asList(applesString));
		...
		while(true) {
			...
			if(judgeString.startsWith(FINISHED)) {
				System.out.println(n+judgeString);
				break;
			}
			...
			System.out.println(greenApple + n);
			...
			if(!judge) {
				...
				String input=br.readLine();
				choice = Integer.parseInt(input);
				outToServer.writeBytes(hand.get(choice)+n);
				hand.remove(choice);
				System.out.println(Waiting for other playersn);				
			}

			Receive the played apples from server
			String playedApples = (inFromServer.readLine()).replaceAll(#, n);
			System.out.println(nThe following apples were playedn+playedApples);
			...
			System.out.println(winningRedApple + n);
	...
	public Apples2Apples(int numberOfOnlinePlayers) throws Exception {
  		redApples = new ArrayListString(Files.readAllLines(Paths.get(., redApples.txt), StandardCharsets.ISO_8859_1)); 
  		greenApples = new ArrayListString(Files.readAllLines(Paths.get(., greenApples.txt), StandardCharsets.ISO_8859_1));

  		shuffle
		rnd = ThreadLocalRandom.current();
		for(int i=redApples.size()-1; i0; i--) {
			int index = rnd.nextInt(i+1);
			String a = redApples.get(index); redApples.set(index, redApples.get(i)); redApples.set(i, a);  SWAP
		}
		rnd = ThreadLocalRandom.current();
		for(int i=greenApples.size()-1; i0; i--) {
			int index = rnd.nextInt(i+1);
		...
		for(int onlineClient=0; onlineClientnumberOfOnlinePlayers; onlineClient++) {
			Socket connectionSocket = aSocket.accept();
			BufferedReader inFromClient = new BufferedReader(new InputStreamReader(connectionSocket.getInputStream()));
			DataOutputStream outToClient = new DataOutputStream(connectionSocket.getOutputStream());
			String handString = ;
			for(int i=0; i7; i++) { Deal 7 cards to the online Player
				handString = ((handString.compareTo()==0)(handString+;)) + redApples.remove(0); Create String of Cards, separated by ;
			}
			outToClient.writeBytes(handString+n);
		...
		if(numberOfOnlinePlayers  3) {
			for(int i=numberOfOnlinePlayers; i=3; i++) {
				players.add(new Player(i, new ArrayListString(), true));
				for(int j=0; j7; j++) { Deal 7 cards to the Non-online bot
					players.get(i).hand.add(redApples.remove(0));
				}
			}
		}
		Add server player
		players.add(new Player(players.size(), new ArrayListString(), false));
		for(int j=0; j7; j++) { Deal 7 cards to the Player
		...
		while(!finished) {
			...
			System.out.println(Green apple  + playedGreenApple + n);
			for(int i=0; inumberOfOnlinePlayers; i++) {
				players.get(i).outToClient.writeBytes(((judge==i)JUDGENOTJUDGE)+n);
				players.get(i).outToClient.writeBytes(Green apple  + playedGreenApple + n);
			}
			...
			Shuffle the answers
			rnd = ThreadLocalRandom.current();
			for(int i=playedApple.size()-1; i0; i--) {
				int index = rnd.nextInt(i+1);
			...
			for(int j=0; jplayers.size()-1; j++) { Iterate between the submitted apples
				playedApplesString = playedApplesString + #t[+j+] + playedApple.get(j).redApple; Create printable String of apples
			}
			for(int i=0; inumberOfOnlinePlayers; i++) {
				players.get(i).outToClient.writeBytes(playedApplesString+n);
			}
			playedApplesString = playedApplesString.replaceAll(#, n);
			System.out.println(nThe following apples were playedn+playedApplesString);
			...
			String winnerString = ((players.get(winningApple.playerID).isBotBotPlayer) +  ID+winningApple.playerID+
										 won with  + winningApple.redApple);
			System.out.println(winnerString + n);
			for(int i=0; inumberOfOnlinePlayers; i++) {
				players.get(i).outToClient.writeBytes(winnerString+n);
			}
			...
			if(finished) {
				String gameWinnerString = FINISHED +((players.get(gameWinner).isBotBotPlayer) +  ID+gameWinner+
										 won the game);
				Notify online clients that someone won the game
				for(int i=0; inumberOfOnlinePlayers; i++) {
					players.get(i).outToClient.writeBytes(gameWinnerString+n);
				}
				System.out.println(n+gameWinnerString);
			}
